...
def add_user(key: str, name: str, role: str = "user") -> None:
    with get_db() as db:
        db.execute(
            "INSERT OR REPLACE INTO users (key, name, role) VALUES (?, ?, ?)",
            (key, name, role)
        )
...
def log_request(user_key: str, path: str, tokens_in: int, tokens_out: int) -> None:
    with get_db() as db:
        db.execute(
            "INSERT INTO request_logs (user_key, path, tokens_in, tokens_out) VALUES (?, ?, ?, ?)",
            (user_key, path, tokens_in, tokens_out)
        )
...
def get_user_history(user_key: str, limit: int = 50) -> list:
    with get_db() as db:
        rows = db.execute("""
            SELECT created_at, path, tokens_in, tokens_out
            FROM request_logs
            WHERE user_key = ?
            ORDER BY id DESC
            LIMIT ?
...
def get_hourly_counts_today() -> list:
...
def export_all_csv() -> str:
    ...
    with get_db() as db:
        rows = db.execute("""
            SELECT l.created_at, u.name, u.key, l.path, l.tokens_in, l.tokens_out
            FROM request_logs l
            JOIN users u ON l.user_key = u.key
            ORDER BY l.id DESC
        """).fetchall()
    lines = ["time,name,key,path,tokens_in,tokens_out"]
    for r in rows:
        lines.append(f"{r['created_at']},{r['name']},{r['key']},{r['path']},{r['tokens_in']},{r['tokens_out']}")
    return "\n".join(lines)
